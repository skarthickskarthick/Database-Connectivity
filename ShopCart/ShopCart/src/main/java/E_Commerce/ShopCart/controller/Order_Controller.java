...
public class Order_Controller {
    ...
    @GetMapping("/findbypaymentstatus")
    public ResponseEntity<List<Order>> getOrdersByPaymentStatus(@RequestParam(name = "paymentStatus") boolean paymentStatus) {
    ...
    public ResponseEntity<HttpStatus> deleteOrder(@PathVariable("id") Long customerID) { // ✅ Ensure `Long` not `long`
        logger.info("Received request to delete order with CustomerID: {}", customerID);

        Optional<Order> order = orderRepository.findByCustomerID(customerID); // ✅ Fetch order

        if (order.isPresent()) { // ✅ Check if order exists
            orderRepository.deleteById(customerID); // ✅ Use deleteById() instead of deleteByCustomerID()
            logger.info("Order deleted successfully: CustomerID {} | HTTP Statuss: {}", customerID, HttpStatus.OK);
            return new ResponseEntity<>(HttpStatus.OK);
        } else {
            logger.warn("Order not found: CustomerID {} | HTTP Status: {}", customerID, HttpStatus.NOT_FOUND);
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
    }
